...
какую предоставляет пакет `reflect`, но зато работает на порядки [быстрее](https://github.com/koykov/versus/tree/master/inspector2)
...
этих целей разработаны спецаильные bound теги:
* `{% jsonquote %}...{% endjsonquote %}` применяет JSON эеранирование ко всему содержимому.
...
Эти инструкции работают стандартно и предсказуемо, но всё же являются громздкими, поэтому были разработаны удобные
инструкции `break if` и `continue if`, которые работают аналогично:
```
{% for _, v := list %}
  {% continue if v.ID == 0 %}
  {% break if v.Status == -1 %}
...
Это не единственный эксклюзив в прерывнии циклов. Представим себе ситуацию, что во время итерации стало понятным, что
...
реализованаэта именно идея и сейчас доступны операторы:
* `break N`
* `lazybreak N`
...
Для `break/lazybreak N` операторов также доступна интеграция с условиями. Например пример выше можно записать так:
```
...
    {% break 2 if j == 8 %}
    {% lazybreak 2 if j == 7 %}
...
* [dyntpl_vector](https://github.com/koykov/dyntpl_vector) предоставляет возможность использовать функционал векторных пармеров в шаблонах.
...
Не стесняйтесь разрабатывать собственные модули расширения. В этом случае рекумендуется регистрировать их в собственном
