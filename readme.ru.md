...
## Подключение под-шаблонов

Для переиспользования шаблонов есть возможность их подключать внутрь прямо из тела шаблона с помощью оператора `include`.
Просто вызовите `{% include subTplID %}` (пример `{% include sidebar/right %}`) и нужный шаблон будет выполнен прямо
внутри текущего шаблона. Под-шаблон будет использовать контекст родительского для доступа к данным.

Также доступна возможность подключать шаблоны в bash-стиле, через `.`.
